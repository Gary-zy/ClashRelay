...
const sampleNodes = [
  { name: "HK-A", type: "ss", server: "hk.example.com", port: 443, cipher: "aes-128-gcm", password: "pw", latency: 12 },
...
test("生成 relay 配置不会给订阅节点补默认参数或污染链路字段", () => {
  const result = buildClashConfig({
    form: createForm({ dialerProxyGroup: ["HK-A"] }),
    nodes: sampleNodes,
    landingNode: null,
  ...
  assert.equal(result.ok, true);
  const hkNode = result.config.proxies.find((proxy) => proxy.name === "HK-A");
  assert.deepEqual(hkNode, {
    name: "HK-A",
    type: "ss",
  ...
  assert.equal(result.config["global-client-fingerprint"], undefined);
  assert.equal(result.config["external-controller"], undefined);
});

test("落地别名和订阅节点冲突时会阻止生成", () => {
  const result = buildClashConfig({
    form: createForm({ socksAlias: "HK-A", dialerProxyGroup: ["HK-A"] }),
    nodes: sampleNodes,
    landingNode: null,
    completeNode: identity,
    completeNodes: identityList,
  ...
  assert.match(result.message, /落地节点别名.*冲突/);
});

test("自定义规则引用不存在的策略时会阻止生成", () => {
  const result = buildClashConfig({
    form: createForm({
      isDirect: true,
      customRulesText: "DOMAIN-SUFFIX,example.com,不存在的策略",
    }),
    nodes: sampleNodes,
    landingNode: null,
    completeNode: identity,
  ...
  assert.equal(result.ok, false);
  assert.match(result.message, /不存在的策略/);
});
